/**
 * @file Tests del GhostPromptSessionStore y su interacción con el token de cancelación.
 */
...
import { GhostPromptSessionStore } from './sessionStore';
...
describe('GhostPromptSessionStore', () => {
  ...
  it('patchState fusiona y notifica suscriptores', () => {
    const listener = vi.fn<
      (snapshot: ReturnType<GhostPromptSessionStore['getSnapshot']>) => void
    >();
    store.subscribe('ghostPrompt.input', listener);

    store.patchState({ draftText: 'hola' });
