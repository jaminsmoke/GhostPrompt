...
if (!fs.existsSync(scriptPath)) {
  console.error(
    `[GhostPrompt] No se encontró el asset de script compilado: ${scriptPath}`,
  );
  process.exit(1);
}
...
if (!body.includes("acquireVsCodeApi")) {
  console.error(
    "[GhostPrompt] Bundle React webview sin firma esperada (acquireVsCodeApi).",
  );
  process.exit(1);
}

console.log(
  `[GhostPrompt] Webview bundle OK (${scriptPath}, ${body.length} bytes)`,
);
