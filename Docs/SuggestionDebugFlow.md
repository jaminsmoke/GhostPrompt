...
- `request-empty`: no hay suggestion (`no-model`, `no-non-premium-model`, `empty-response`, `premium-quota-blocked`).
...
- `reason=no-non-premium-model`: cambiar policy a `anyModel` o mantener modo seguro.
