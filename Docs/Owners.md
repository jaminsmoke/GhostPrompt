...
| **`engines/`**          | Motores `CompletionProvider` (`copilot/`, `opencode/`, `ollama/`) + `engineRegistry.ts` + **`config/completionSources.ts`** (fuentes habilitadas) + **`routing/resolveCompletionSource.ts`** + **`catalog/mergedModelCatalog.ts`** (lista unificada para settings/UI).                                                                                                                                                                                                                     | `sugcore/` (types, `prompt/`, etc. vía imports de motor), `system/log/`, `vscode`. | No importar desde `api/` como capa superior (evitar ciclos).            |
...
| **`system/internals/protocols/`** | Contratos puros: constantes, tipos, guards, estado declarativo, schemas Zod (`zschemWebviewMessages.ts`).                                                                                                                                                                                                                                                                          | `zod` (solo en `validations/schemas/`).                                                                                                               | Sin `vscode`; parsers con logging en `api/protocols/`.                  |
| **`system/build/`**     | Scripts de verificación empaquetado (ej. webview bundle).                                                                                                                                                                                                                                                                                                                             | Node.                                                                                                                                                 | —                                                                       |

### ESLint (`import/no-restricted-paths`)

Reglas en `eslint.config.js` (nivel `error`):

| Objetivo (`target`)       | No importar desde (`from`)                                              |
| ------------------------- | ----------------------------------------------------------------------- |
| **`src/sugcore/**/\*`**   | **`src/ui/provider/**/\*`**, **`src/ui/notifications/**/\*`**, **`src/api/**/\*`** |
...
| `ui/` (webview, provider, notifications)                                                                              | `MiniInputViewProvider.test.ts`, `webviewToolbarParity.test.ts`, `webviewThemeTokens.test.ts`, `src/ui/webview/react/App.test.tsx`, `host/suggestionHostNotification.test.ts`                                                                                                                                                                          |
| `api/protocols/*` (webviewProtocols, inboundHandlers)                                                                 | `api/protocols/webviewProtocols.test.ts`, `api/protocols/ghostPromptWebviewInboundHandlers.test.ts`, `system/internals/protocols/validations/schemas/zschemWebviewMessages.test.ts`                                                                                                                                                               |
...
| `engines/` (copilot, opencode, ollama, engineRegistry, **config/completionSources**, **routing/resolveCompletionSource**, **catalog/mergedModelCatalog**) | `engineRegistry.test.ts`, `completionProvider.test.ts`, `resolveCompletionSource.test.ts`, `ollamaLmEngine.test.ts`, `ollamaApiClient.test.ts`, `mergedModelCatalog.test.ts`, `opencodeApiClient.test.ts`, `opencodeLmCompletion.test.ts`, `opencodeModelCatalog.test.ts`, `opencodeModelTier.test.ts`, `normalizeOpencodeProviderModels.test.ts` |
