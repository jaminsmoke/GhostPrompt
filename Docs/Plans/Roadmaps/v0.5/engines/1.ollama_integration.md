...
| **Pendiente** | ¿Hay que migrar modelos OpenCode a `engines/opencode/` como archivos separados (`apiClient`) o se quedan dentro de `src/opencode/`? | Se queda como está; `src/opencode/` tiene runtime/SDK/CLI que es infraestructura compartida. Solo el _adapter_ de completion se mueve a `engines/opencode/`. |
