...
import { SuggestionRequestGovernor } from '../src/system/policies/SuggestionRequestGovernor';
...
describe('SuggestionRequestGovernor', () => {
  ...
  it('sirve desde cache para mismo input normalizado', () => {
    ...
    if (first.kind === 'request') {
      governor.saveResult(first.key, { kind: 'suggestion', suggestion: 'continuacion' }, config);
    }
  ...
  it('no reutiliza caché si cambia la huella del bootstrap del proyecto', () => {
    ...
    expect(first.kind).toBe('request');
    if (first.kind === 'request') {
      governor.saveResult(first.key, { kind: 'suggestion', suggestion: 'v1' }, relaxedConfig);
    }
