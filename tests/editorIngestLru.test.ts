...
function ed(path: string, last: number, prompt = 'x'): ProjectMemoryEditorIngestStoredItem {
  const z = '0'.repeat(64);
  return {
...
describe('applyEditorIngestLruEviction', () => {
  ...
  it('conserva una única entrada cuando cabe en recuento y techo de bytes', () => {
    ...
    expect(applyEditorIngestLruEviction(items, 5, 500_000).map((e) => e.relativePath)).toEqual([
      'a.ts',
    ]);
  });
  ...
  it('evita por bytes totales aproximados cuando hay payload grande', () => {
    ...
    const items = [ed('old.ts', 1, big), ed('new.ts', 100, big), ed('mid.ts', 50, big)];
