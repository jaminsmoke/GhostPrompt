...
describe('CopilotCompletion', () => {
  ...
  it('buildCompletionInstruction incluye directiva de estilo y guía de continuación', () => {
    const instruction = buildCompletionInstruction('Escribe una propuesta', 'detailed', {
      ...
      recentSentPrompts: ['Diseña un backend con Node y PostgreSQL', 'Quiero tests para API REST'],
    ...
    expect(instruction).toContain('Partial text to continue: Escribe una propuesta');
    expect(instruction).toContain('no leading space');
    expect(instruction).toContain('comm');
    expect(instruction).toContain('itmentes');
    expect(instruction).not.toContain('Recent prompt sent by user');
  });
  ...
  it('detecta idioma espanol e ingles de forma basica', () => {
    expect(detectSuggestionLanguageFromInput('Quiero una funcion para validar email')).toBe('es');
    expect(detectSuggestionLanguageFromInput('I need a function to validate email')).toBe('en');
  });
  ...
  it('manual tiene precedencia sobre auto en resolucion de idioma', () => {
    expect(resolveSuggestionLanguage('manual', 'en', 'Quiero una propuesta')).toBe('en');
    expect(resolveSuggestionLanguage('auto', 'en', 'Quiero una propuesta')).toBe('es');
  });
