...
} from "../src/completion/context/projectBootstrapContext";
...
describe("fingerprintProjectBootstrapLines", () => {
  ...
  it("es estable para el mismo contenido", () => {
    ...
    expect(fingerprintProjectBootstrapLines(lines)).toBe(
      fingerprintProjectBootstrapLines(lines),
    );
  });
    ...
    readFileMock.mockImplementation((fsPath: string) => {
      ...
      if (fsPath.endsWith("package.json")) {
        return Promise.resolve(
          enc.encode(JSON.stringify({ name: "pkg", version: "1.0.0" })),
        );
      }
