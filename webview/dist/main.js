...
(() => {
  ...
  function composeContextShort(v) {
    if (v === "project") {
      return "Proyecto";
    }
    if (v === "off") {
  ...
  function composeLangShort(choice, effective) {
    if (choice === "auto") {
      const code = effective === "es" ? "ES" : effective === "en" ? "EN" : "...";
      return `Auto (${code})`;
    }
  ...
  var webviewSettingsPayloadSchema = external_exports.object({
    ...
    suggestionStyle: external_exports.enum(["concise", "balanced", "detailed"]),
    contextMode: external_exports.enum(["off", "basic", "project"]),
  ...
  var webviewUpdateSettingSchema = external_exports.discriminatedUnion("key", [
    external_exports.object({
      type: external_exports.literal("updateSetting"),
      key: external_exports.literal("suggestionModelPolicy"),
      value: external_exports.enum(["nonPremiumOnly", "anyModel"])
    }),
    external_exports.object({
      type: external_exports.literal("updateSetting"),
      key: external_exports.literal("selectedModelId"),
      value: external_exports.string()
    }),
    external_exports.object({
      type: external_exports.literal("updateSetting"),
      key: external_exports.literal("suggestionStyle"),
      value: external_exports.enum(["concise", "balanced", "detailed"])
    }),
    external_exports.object({
      type: external_exports.literal("updateSetting"),
      key: external_exports.literal("contextMode"),
      value: external_exports.enum(["off", "basic", "project"])
    }),
    external_exports.object({
      type: external_exports.literal("updateSetting"),
      key: external_exports.literal("suggestionLanguageChoice"),
      value: external_exports.enum(["auto", "es", "en"])
    }),
    external_exports.object({
      type: external_exports.literal("updateSetting"),
      key: external_exports.literal("debugSuggestions"),
      value: external_exports.boolean()
    })
  ]);
  var webviewInboundMessageSchema = external_exports.union([
    external_exports.object({ type: external_exports.literal("init") }),
    external_exports.object({
      type: external_exports.literal("suggest"),
      text: external_exports.string(),
      captureId: external_exports.number()
    }),
    external_exports.object({
      type: external_exports.literal("draftChanged"),
      text: external_exports.string(),
      originViewId: external_exports.string().min(1)
    }),
    external_exports.object({
      type: external_exports.literal("accept"),
      context: external_exports.string(),
      suggestion: external_exports.string()
    }),
    external_exports.object({
      type: external_exports.literal("send"),
      text: external_exports.string()
    }),
  ...
  (function() {
    ...
    function refreshComposeSummary() {
      ...
      if (!s || typeof s !== "object") {
        sumEl.textContent = "Estilo \xB7 Contexto \xB7 Idioma";
        return;
      }
      sumEl.textContent = `${composeStyleShort(String(s.suggestionStyle ?? ""))} \xB7 ${composeContextShort(String(s.contextMode ?? ""))} \xB7 ${composeLangShort(String(s.suggestionLanguageChoice ?? ""), String(s.effectiveSuggestionLanguage ?? ""))}`;
    }
    ...
    function renderInlineGhost() {
      ...
      ghostInline.innerHTML = `<span class="typed">${typed}</span><span class="suggestion">${suggestion}</span>`;
    ...
    function syncComposerHeight() {
      ...
      const renderedSuggestion = ghostVisible ? buildInsertedSuggestion(input.value, pendingSuggestion) : "";
      const combined = ghostVisible ? `${input.value}${renderedSuggestion}` : input.value;
    ...
    window.addEventListener("message", (event) => {
      ...
      if (typeof message.captureId === "number" && message.captureId !== currentCaptureId && message.broadcast !== true) {
        return;
      }
      if (message.type === "loading") {
        const loadingLabel = typeof message.statusText === "string" && message.statusText.trim().length > 0 ? message.statusText : "Buscando sugerencia...";
        ...
        lastCompletionProvider = settings.completionProvider === "opencode" ? "opencode" : "copilot";
        lastCompletionUiKind = typeof settings.completionUiKind === "string" ? settings.completionUiKind : lastCompletionProvider === "opencode" ? "opencode" : "copilot";
    ...
    function setLanguageAutoLabel(effectiveLanguage) {
      ...
      const code = effectiveLanguage === "es" ? "ES" : effectiveLanguage === "en" ? "EN" : "...";
      autoChip.textContent = `Auto (${code})`;
    }
    function setModelOptions(availableModels, selectedModelId, _completionProvider, completionUiKind) {
      if (!(modelSelect instanceof HTMLSelectElement)) {
        return;
      }
      const uiKind = typeof completionUiKind === "string" ? completionUiKind : "copilot";
      ...
      const sortedProviders = Array.from(byProvider.keys()).sort((a, b) => {
        ...
        const stripPrefix = (k) => {
          if (k.startsWith("Copilot \xB7 ")) {
            return k.slice("Copilot \xB7 ".length);
          }
          if (k.startsWith("OpenCode \xB7 ")) {
            return k.slice("OpenCode \xB7 ".length);
          }
          return k;
        };
      ...
      const providerOptions = sortedProviders.map((provider) => {
        const rows = (byProvider.get(provider) || []).slice().sort((a, b) => {
          const la = typeof a?.label === "string" ? a.label.toLowerCase() : String(a?.id ?? "");
          const lb = typeof b?.label === "string" ? b.label.toLowerCase() : String(b?.id ?? "");
      ...
      modelSelect.innerHTML = `<option value="auto">${escapeHtml(autoLabel)}</option>` + providerOptions;
    ...
    function setRuntimeModelLabel(model) {
      ...
      const m = model;
      const labelRaw = typeof m.label === "string" && m.label.trim() ? m.label.trim() : typeof m.id === "string" ? m.id : "unknown";
      const tier = m.tier;
      const pricing = typeof m.pricing === "string" ? m.pricing.trim() : "";
      const provider = typeof m.provider === "string" && m.provider.trim() ? `${m.provider.trim()} \xB7 ` : "";
      modelRuntimeLabel.textContent = `Modelo: ${provider}${labelRaw} \xB7 ${formatTierToken(tier, pricing, lastSuggestionUiLang)}`;
    }
    function formatProviderTitle(provider) {
      return `\u25B8 ${provider}`;
    }
    function formatTierToken(tier, pricing, lang) {
      ...
      if (tier === "included") {
        return L === "es" ? `[Gratis${normalizedPricing}]` : `[INCLUDED${normalizedPricing || " 0x"}]`;
      }
      return L === "es" ? `[Sin clasificar${normalizedPricing}]` : `[UNKNOWN${normalizedPricing}]`;
    }
  })();
})();
